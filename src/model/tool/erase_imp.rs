        ...
        for y in 0..self.size {
            for x in 0..self.size {
                match self.brush_type {
                    ...
                    EraseType::Solid => {
                        editor.set_char(center + Position::new(x, y), Some(AttributedChar::new(' ', TextAttribute::default())));
                    }
                }
            }                
        }
...
impl Tool for EraseTool
{
    ...
    fn show_ui(&mut self, _ctx: &egui::Context, ui: &mut egui::Ui, _buffer_opt: Option<std::sync::Arc<std::sync::Mutex<crate::ui::ansi_editor::BufferView>>>)
    {
        ui.horizontal(|ui| {
