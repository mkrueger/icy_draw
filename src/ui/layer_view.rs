...
use eframe::{egui::{self, RichText}, epaint::Vec2};
use egui_extras::{TableBuilder, Column};
use i18n_embed_fl::fl;
use icy_engine::{AsciiParser, BufferParser};
use std::sync::{Arc, Mutex};

pub fn show_layer_view(ctx: &egui::Context, ui: &mut egui::Ui, buffer_opt: Option<Arc<Mutex<BufferView>>>) {

    if buffer_opt.is_none() { return; }
                            ...
                            row.col(|ui| {
                                if ui
                                    .add(egui::ImageButton::new(
                                        if layer.is_visible { super::VISIBLE_SVG.texture_id(ctx) } else { super::INVISIBLE_SVG.texture_id(ctx) },
                                        Vec2::new(16., 16.),
                                    ))
                                    .on_hover_ui(|ui| {
                                        ui.label(
                                            RichText::new(fl!(crate::LANGUAGE_LOADER, "move_layer_up_tooltip"))
                                                .small(),
                                        );
                                    })
                                    .clicked()
    ...
    ui.horizontal(|ui| {
        if ui
            ...
            .on_hover_ui(|ui| {
                ui.label(
                    RichText::new(fl!(crate::LANGUAGE_LOADER, "add_layer_tooltip"))
                        .small(),
                );
            })
            .clicked()
        ...
        .on_hover_ui(|ui| {
            ui.label(
                RichText::new(fl!(crate::LANGUAGE_LOADER, "delete_layer_tooltip"))
                    .small(),
            );
        })
        .clicked()
